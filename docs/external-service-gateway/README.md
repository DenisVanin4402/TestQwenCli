...
sequenceDiagram
    ...
    G->>DB: try acquire slot, max 5 for sync
    alt slot acquired
...
Sync-запрос не обязан попадать в persistent queue. Он участвует в общей политике лимитов через Slot Manager и короткоживущую запись в `ext_sync_waiters`. Для аудита можно писать lightweight-запись в `ext_call_audit`.
...
sequenceDiagram
    ...
    G->>DB: mark DONE, save result,<br/>create callback delivery PENDING atomically
    G->>DB: release slot
...
sequenceDiagram
    ...
    G->>DB: SELECT task by taskId and authenticated client identity
    G-->>C: status + result + error
```

Для `GET`, `DELETE` и ручного retry gateway не принимает `clientService` из параметров запроса. Сервис-клиент определяется из service-to-service identity, а выборка задачи ограничивается парой `taskId` и `clientService`.

Для `POST /v1/external/sync` и `POST /v1/external/async` поле `clientService` в теле запроса должно совпадать с service-to-service identity вызывающего сервиса. Gateway не использует это поле как самостоятельный источник доверия: несовпадение считается ошибкой авторизации или валидации. Это сохраняет явный контракт для idempotency scope и callback allow-list, но не позволяет сервису подставить чужое имя.
...
Опциональные заголовки callback:
...
- polling остается страховкой от потери уведомления.
