...
- [ ] Перед стартом каждого этапа создавать или актуализировать `plan_TXXX.md`.
...
- Stage-level senior architect reviews `review_TXXX.md` только запланированы.
...
- Начать с CR002-T001: создать `plan_T001.md`, затем составить матрицу фактических endpoints и DTO по контроллерам.
- Перед закрытием CR002-T001 создать `review_T001.md` и обработать замечания после human approval.
