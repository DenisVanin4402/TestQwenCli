...
- переноса request-level idempotency sync/async на внутреннюю библиотеку `@Idempotent` после ее подключения;
...
## Допущение CR003: система находится в разработке

CR003 выполняется до production-запуска системы. Production-данные не мигрируются.

Следствия:

- изменения схемы и контрактов внутренних таблиц можно делать прямым dev-phase переходом, если публичный HTTP API остается согласованным с текущими CR;
- миграции должны описывать целевую схему для воспроизводимого старта тестовых и будущих окружений, но downtime-free data migration не требуется;
- rollback допускается через `git revert` и пересоздание локальной/Testcontainers БД;
- защитные DB constraints и unique indexes все равно сохраняются как нижний слой корректности.
...
| Метод | Ключ `@Idempotent` | Hash fields | Не входит в hash | DB guard |
| --- | --- | --- | --- | --- |
| `ExternalAsyncServiceImpl.submit` | `request.clientService + request.externalId` | `request.payload`, `request.priority`, `request.deliveryMode` | `requestId` | async unique index в `ext_request_queue` |
| `ExternalSyncServiceImpl.sync` | `request.clientService + headers.idempotencyKey` | `request.externalId`, `request.payload` | `headers.requestId` | sync unique key в `ext_request_queue` после CR004 |

Если `headers.idempotencyKey` для sync отсутствует, CR003-T001 должен явно выбрать поведение: отклонять sync-запрос как неидемпотентный, пропускать без idempotency или использовать резервный ключ. Предпочтение для надежного sync API - требовать ключ для идемпотентного режима.
...
| --- | --- | --- | --- |
| 1 | CR003-T001: упрощение request-level idempotency через `@Idempotent` | P0 | Ручная async submit idempotency переносится на будущую аннотацию, DB unique guard остается, sync idempotency учитывается через `ext_request_queue`. |
| 2 | CR003-T002: инвентаризация ядра и DB-операций | P0 | Есть карта повторяющегося dispatcher/retry/JDBC-кода и классификация DB-операций на критичные и не критичные. |
| 3 | CR003-T003: общий dispatch loop | P0 | Async dispatcher и callback dispatcher используют общий каркас batch-dispatch без изменения доменной обработки. |
| 4 | CR003-T004: управляемые Spring executors | P0 | Ручные `Executors.newFixedThreadPool` заменены на Spring-managed executor beans с сохранением bounded parallelism. |
| 5 | CR003-T005: общий retry decision | P0 | Retry/backoff/dead decision выделен в общий компонент, но async и callback статусы остаются явными. |
| 6 | CR003-T006: уменьшение ручного JDBC-маппинга | P1 | Повторяющиеся row/json/status mapping helpers унифицированы без изменения SQL-атомарности. |
| 7 | CR003-T007: JPA-дизайн для не критичных DB-операций | P1 | Зафиксировано, какие операции можно безопасно перевести на Spring Data JPA Repository и какие нельзя. |
| 8 | CR003-T008: экспериментальный JPA read/non-critical adapter | P2 | При подтверждении T007 добавлен или спланирован `postgres-jpa` режим для не критичных операций без production-переключения. |
| 9 | CR003-T009: spike критичных JPA/native операций | P2 | Доказано или отклонено использование JPA custom fragments/native queries для hot-path операций. |
| 10 | CR003-T010: parity, contract guardrails и concurrency проверки | P0 | Рефакторинг проходит существующие contract, e2e и PostgreSQL concurrency tests; при JPA добавлены отдельные parity tests; после generated OpenAPI client/server сохранены только нужные contract/code guardrails. |
| 11 | CR003-T011: проверка архитектурной документации и ADR | P1 | Зафиксировано, меняются ли архитектурные документы; при необходимости добавлен ADR. |
| 12 | CR003-T012: финальная проверка | P0 | `mvn test` проходит, а для DB-изменений запущен или явно отложен `mvn verify -Pintegration-tests`. |

## Детализация задач

### CR003-T001: упрощение request-level idempotency через `@Idempotent`

Цель: убрать ручную прикладную обвязку async submit idempotency и подготовить sync request idempotency через lifecycle-запись в `ext_request_queue`.
...
- Сопоставить операции из `inventory_T002.md` с возможностями Spring Data JPA Repository, projections, `@Lock`, `@QueryHints`, `@Modifying`, native queries и custom fragments.
...
Цель: если T007 подтверждает пользу, добавить JPA только там, где риск минимален.
