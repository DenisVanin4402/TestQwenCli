...
| `.spec-diff/` | Diff metadata branch-diff change | `openspec-change-from-diff` | refs, name-status, stat, patch, changed-files.yaml, context-files.yaml |
...
Branch-diff цепочка для сценария, где аналитик меняет master-spec documents в отдельной ветке:

```text
teach -> init-master-spec -> change-from-diff -> review/approve? -> design -> implement -> verify source -> archive-change
```
...
4. `openspec-propose` создает `change.md`, работая от master-spec folder и manifest.
   Если master-spec уже изменена аналитиком в отдельной ветке, вместо `openspec-propose` используется `openspec-change-from-diff`.
...
### 4.1. Branch-diff workflow

Branch-diff workflow используется, когда аналитик меняет `openspec/<service>/` напрямую в отдельной git-ветке, а разработчик в любой своей текущей ветке генерирует аналитический `change.md` из diff.

Основной сценарий:

1. Есть базовая локальная ветка/ref, например `release/2026-06`.
2. Аналитик создает ветку от базовой, например `analysis/add-callback-retry`.
3. Аналитик меняет файлы в `openspec/<service>/` и коммитит изменения.
4. Разработчик запускает `openspec-change-from-diff` с параметрами `service`, `base_ref`, `analyst_ref`, `change_name`.
5. Framework проверяет только локальные refs через `git rev-parse --verify`.
6. Framework выполняет diff только по pathspec `-- openspec/<service>/`.
7. Framework читает changed/context files через `git show <ref>:<path>`, без checkout base/analyst веток.
8. Framework создает `openspec/changes/<change-name>/change.md` и при включенных artifacts — `.spec-diff/`.
9. После реализации финальный `apply-change` по умолчанию заменяется verify-проверкой source-of-truth docs в analyst ref.
...
Three-dot сравнивает изменения ветки аналитика относительно merge-base с базовой веткой. Explicit override `diff_mode=two-dot` допустим только по явному выбору пользователя:

```bash
git diff --find-renames --find-copies <base_ref>..<analyst_ref> -- openspec/<service>/
```

Artifacts branch-diff:
...
`.spec-diff/changed-files.yaml` фиксирует все added/modified/deleted/renamed/copied/typechanged файлы master-spec root. `.spec-diff/context-files.yaml` фиксирует документы, выбранные через manifest по `depends_on`, `related_files`, общим `entities`, `integrations`, `endpoints`, `events` и `read_priority=high`.
...
|---|---|---|
| `На согласовании` | `openspec-propose` / `openspec-change-from-diff` | Change создан и отправляется на ревью |
...
Если пользователь просит создать change из diff веток, по base branch и analyst branch или сообщает, что ветка аналитика уже изменила master spec, использовать `openspec-change-from-diff`, а не обычное интервью `openspec-propose`.

### `openspec-change-from-diff`

Создает `change.md` из git diff двух локальных refs по `openspec/<service>/`.

Важные правила:

- обязательные inputs: `service`, `base_ref`, `analyst_ref`, `change_name`;
- default `diff_mode=three-dot`;
- refs проверяются только локально через `git rev-parse --verify`;
- framework не делает fetch/pull и не переключает ветки;
- changed files читаются через `git show <ref>:<path>`;
- контекст выбирается через manifest;
- `.spec-diff/` сохраняет refs, name-status, stat, patch, changed-files и context-files;
- `Spec update mode` в созданном `change.md` всегда `branch-diff`.
...
- Для `Spec update mode: branch-diff` сначала читает `.spec-diff/changed-files.yaml`, если файл есть, и добавляет diff metadata в "Источники требований".
- Если текущая рабочая ветка не содержит master-spec docs из analyst ref, читает нужные документы через `git show <analyst_ref>:<path>`.
...
- Для `branch-diff` в итоговом отчете указывает, что source-of-truth master-spec изменения находятся в `Analyst ref`.
...
1. Не изменять master-spec документы.
2. Проверить локальные `Base ref`, `Analyst ref`, `Merge base`.
3. Повторить diff summary по `openspec/<service>/`.
4. Сравнить текущий summary с `.spec-diff/changed-files.yaml`.
5. Проверить, что master-spec изменения существуют в `analyst_ref`.
6. Если текущая ветка должна содержать эти docs, использовать `git diff --quiet <analyst_ref> -- openspec/<service>/` и выводить warning при несовпадении.
7. Explicit apply `git checkout <analyst_ref> -- openspec/<service>/` разрешен только после clean status master-spec root и явного подтверждения пользователя.

### `openspec-archive-change`

Архивирует директорию change целиком в `openspec/changes/archive/YYYY-MM-DD-<name>/`. Для branch-diff change `.spec-diff/` переносится вместе с change как часть истории генерации.
...
Для `Spec update mode: branch-diff` также обязательны:
...
Для branch-diff change раздел `## 0. Источники изменения` фиксирует Git refs, changed files, context files и ссылки на `.spec-diff/` artifacts.
...
- `openspec-change-from-diff` создает change из двух локальных refs без checkout веток.
...
- `openspec-apply-change` для `branch-diff` выполняет verify и предупреждения, а не автоматический merge.
...
| Локальные refs для branch-diff отсутствуют | Не делать fetch автоматически, попросить пользователя подготовить refs вручную |
| Ветка аналитика изменилась после генерации `change.md` | Повторить diff summary и сравнить с `.spec-diff/changed-files.yaml` |
...
3. Proposal: оформить change как аналитический delta-документ:
   - через `openspec-propose`, если требования формируются в текущем процессе;
   - через `openspec-change-from-diff`, если analyst ref уже содержит измененную master specification.
