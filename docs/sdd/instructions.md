...
## Часть 10: Сценарий "From Diff" — генерация delta-spec из MR

### 10.1 Что это и когда использовать

Сценарий **"From Diff"** — альтернативный point входа в SDD-фреймворк. Вместо того чтобы
пользователь описывает требование и создаётся спецификация с нуля (`/sdd:specify`),
аналитик вносит изменения в MR, и на основе git diff генерируется **delta-spec** —
спецификация, содержащая ТОЛЬКО изменения.

**Когда использовать:**
- Системный аналитик внёс бизнес-изменения в проект (документация, контракты, код)
- Нужно быстро понять что изменилось без чтения всех файлов MR
- Разработчикам/AI-агентам нужен структурированный контекст для реализации

**Когда НЕ использовать:**
- MR содержит только технические изменения (refactor, formatting) без бизнес-логики
- Пользователь описывает новую фичу с нуля (использовать `/sdd:specify`)
- Delta-spec уже существует для данного MR

### 10.2 Сравнение: From Prompt vs From Diff

| Аспект | From Prompt (`/sdd:specify`) | From Diff (`/sdd:from-diff`) |
|--------|------------------------------|-------------------------------|
| Вход | User request (текстовое описание) | Git diff (ветки или коммиты) |
| Выход | `spec.md` — полная спецификация | `delta-spec.md` — только изменения |
| Требования | REQ-1, REQ-2, ... | REQ-NEW-1, REQ-NEW-2, ... |
| AC | AC-1, AC-2, ... | AC-NEW-1, AC-NEW-2, ... |
| После создания | Plan → Tasks → Tests → Impl → Review | Plan → Tasks → Tests → Impl → Review |

Оба пути сходятся после создания спецификации.

### 10.3 Пошаговый пример

**Шаг 1: Аналитик внёс изменения в MR**

```bash
# Ветка аналитика с изменениями бизнес-требований
git checkout feature-analyst-2024
# ... аналитик изменил openapi.yaml, docs/rules.md, application.properties
git commit -m "docs: обновить требования к API авторизации"
```

**Шаг 2: Запуск diff-extractor**

```bash
# Извлечь diff и классифицировать изменения
bash scripts/sdd-diff-extract.sh main feature-analyst-2024 docs/specs/0002-sdd-from-diff/
```

Результат:
- `diff-summary.md` — список изменённых файлов с классификацией
- `impact-map.md` — карта влияния

**Шаг 3: Генерация delta-spec через команду**

```
/sdd:from-diff main feature-analyst-2024
```

Команда:
1. Запускает diff-extractor → получает `diff-summary.md` и `impact-map.md`
2. Анализирует изменённые файлы
3. Генерирует `delta-spec.md` с REQ-NEW-* и AC-NEW-*
4. Создаёт `plan.md` и `tasks.md`
5. Отчитывается о созданных файлах

**Шаг 4: Продолжение pipeline**

После создания delta-spec — тот же pipeline что и From Prompt:
```
/sdd:clarify → уточнить неоднозначности
/sdd:tasks   → если нужно разбить детальнее
/sdd:implement → реализовать задачи
/sdd:review → верифицировать соответствие
```

### 10.4 Формат delta-spec

Delta-spec (`docs/specs/_template/delta-spec.md`) содержит:

| Секция | Описание |
|--------|----------|
| **Метаданные** | Источник MR, аналитик, дата, ветки, commit range |
| **Источник изменений** | Таблица всех изменённых файлов с типом (A/M/D) |
| **Проблема** | Описание бизнес-проблемы из MR |
| **НОВЫЕ требования (REQ-NEW-*)** | Только новые требования, не повторять существующие |
| **НОВЫЕ acceptance criteria (AC-NEW-*)** | Критерии приёмки для новых требований |
| **ИЗМЕНЁННЫЕ NFR** | Какие NFR затронуты (или «не изменяются») |
| **Затронутые существующие требования** | Ссылки на REQ из основного spec, которые затронуты |
| **Матрица трассировки (delta)** | REQ-NEW → AC-NEW → файлы из diff → статус |
| **Связанные документы** | ADR, спеки, контракты, которых касаются изменения |
| **Impact Map summary** | Прямое и косвенное влияние по областям |

**Важно:** Delta-spec — это изменения (delta), а не новая полная документация.
При слиянии в основной spec номера REQ-NEW-* заменяются на сквозные REQ-N.

### 10.5 Использование diff-extractor

**Команды:**

```bash
# Linux/macOS
bash scripts/sdd-diff-extract.sh <ref1> <ref2> [target-spec-dir]

# Windows
scripts\sdd-diff-extract.bat <ref1> <ref2> [target-spec-dir]
```

**Аргументы:**
- `ref1` — базовая ветка или коммит (например, `main`)
- `ref2` — ветка или коммит с изменениями (например, `feature-xyz`)
- `target-spec-dir` (опционально) — директория для копирования результатов

**Что создаёт:**

| Файл | Содержимое |
|------|------------|
| `diff-summary.md` | Статистика изменений, классификация по типам (контракты/требования/код/конфиги), полный список файлов |
| `impact-map.md` | Прямое влияние (файлы из diff), косвенное влияние (рекомендуемые проверки), затронутые спеки и ADR |

**Классификация файлов:**

| Тип | Паттерны | Примеры |
|-----|----------|---------|
| Контракты | `openapi.yaml`, `asyncapi.yaml`, `*.dbml` | API-спецификации, схемы БД |
| Требования | `docs/*.md`, `*.md` | Бизнес-требования, правила |
| Код | `src/*.java`, и т.д. | Исходный код |
| Конфигурации | `*.properties`, `*.yml`, `pom.xml` | Настройки приложения |

### 10.6 Qwen команда и навык

**Команда `/sdd:from-diff`** — определена в `.qwen/commands/sdd/from-diff.md`.
Принимает ветки/коммиты для сравнения, запускает diff-extractor, генерирует delta-spec.

**Навык `sdd-diff-analyzer`** — определён в `.qwen/skills/sdd-diff-analyzer/SKILL.md`.
Описывает процедуру анализа diff, классификации изменений и построения карты влияния.
Подгружается автоматически при необходимости анализа diff.
