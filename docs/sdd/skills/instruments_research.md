...
11. Многоуровневая защита destructive actions: policy в `AGENTS.md`, protected branches/CI, secret scan, lint SQL/migrations и task-runner обертки для опасных операций.
12. Локальные `pre-commit` hooks можно использовать как дополнительную страховку, но не считать security boundary: они не покрывают прямые shell/DB/GUI/cloud actions и могут быть отключены локально.
...
- Зафиксировать policy в `AGENTS.md`: какие операции считаются destructive, когда нужно явное подтверждение, какой dry-run или diff нужно показать перед выполнением.
- Настроить protected branches и CI-gates для merge/push: тесты, formatter/linter, запрет force push в protected ветки, обязательный review для рискованных изменений.
- Добавить secret scan и lint SQL/migrations: блокировать секреты, `DROP`/`TRUNCATE`, массовые `DELETE`/`UPDATE` и миграции без явного review/rollback-плана.
- Для опасных операций ввести обертки через task runner (`task`, `make`, Maven profile, script) с явными флагами вроде `--dry-run` и `--confirm`.
- Добавить локальный `pre-commit` как страховку для быстрых проверок, но не считать его security boundary: его можно обойти, и он не видит действия вне Git.
