...
import com.example.testqwencli.gateway.async.ExternalAsyncDispatcher;
...
class CallbackDeliveryFlowTest {
	...
	private static CallbackDeliveryDispatcher callbackDispatcher(
			MemoryAsyncTaskRepository taskRepository,
			MemoryCallbackDeliveryRepository deliveryRepository,
			RecordingCallbackClient callbackClient,
			Clock clock,
			int maxAttempts
	) {
		return new CallbackDeliveryDispatcher(deliveryRepository, taskRepository, callbackClient,
	...
	private static ExternalGatewayAsyncProperties asyncProperties() {
		return new ExternalGatewayAsyncProperties(Duration.ofMillis(100), 3, false, ASYNC_RETRY_BACKOFF);
	}

	private static ExternalGatewayCallbackProperties callbackProperties(int maxAttempts) {
		return new ExternalGatewayCallbackProperties(false, maxAttempts, CALLBACK_RETRY_BACKOFF,
